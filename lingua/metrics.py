...
def upload_train_to_wandb(
    ckpt_dir, project="lingua", entity=None, train=True, eval=True
):
    import wandb
...
def get_num_params(model: nn.Module, exclude_sdm_memory: bool = True) -> int:
    """
    Get the total model params.

    By default the SDM learned memory bank is excluded from the count: it is the model's
    *state* (read sparsely, ~no FLOPs), not an "active" parameter, so it should not inflate
    the reported model size or the 6*N FLOP-per-token estimate.
    """
    numel = {
        n: p.numel()
        for n, p in model.named_parameters()
        if not (exclude_sdm_memory and getattr(p, "_sdm_memory_bank", False))
    }
