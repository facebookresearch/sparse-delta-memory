...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        if args.checkpoint.init_ckpt_path:
            ...
            # RoPE tables are non-persistent buffers (not in the checkpoint); recompute the
            # default table and any per-head_dim SWA tables after loading.
            model.rope_embeddings.reset_parameters()
            for _rope in model.extra_rope.values():
                _rope.reset_parameters()
        else:
        ...
        # log model size
        sdm_memory_params = get_num_params(model, exclude_sdm_memory=False) - model_param_count
        logger.info(
            f"Model size: {model_param_count:,} active parameters"
            + (f" (+ {sdm_memory_params:,} SDM memory-bank params)" if sdm_memory_params else "")
        )
